...
from ultralytics.nn.addmodules.afpn_modules import *
from ultralytics.nn.addmodules.bifpn import *
...
def parse_model(d, ch, verbose=True):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        """模型深度调整为对应的 n * depth 初始深度乘深度控制变量1"""
        ...
        if m in {
            ...
            c1, c2 = ch[f], args[0]
            """确保网络输出通道数（c2）满足分类任务的需求（等于 nc）。限制通道数不超过 max_channels。对通道数进行硬件友好的调整（对齐到 8 的倍数）。"""
            ...
            """生成最终真实的模型层的参数 从 输入 输出 后开始替换  *表示解包操作"""
            args = [c1, c2, *args[1:]]

            """以下通过不同的模块名修改args中的参数及c2"""
            if m in {
                BottleneckCSP,
                C1,
                C2,
                ...
                n = 1
            if m in {C3k2,C3k2_DWSConv}:  # for M/L/X sizes
                legacy = False
                if scale in "mlx":
                    args[3] = True
        elif m is AIFI:
            ...
            args[0] = [int(x * width) for x in args[0]]
            c2 = args[0][0]
        elif m is ResUpDownSample:
            c1 = ch[f]
            c2 = int(args[0]*width)
            args = [c1, c2, *args[1:]]
        elif m is ACBiFPN:
