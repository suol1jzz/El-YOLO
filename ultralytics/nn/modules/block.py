...
from .conv import Conv, DWConv, GhostConv, LightConv, RepConv, autopad,DWSConv
...
class Bottleneck_DWSConv(nn.Module):
    def __init__(self, c1, c2, shortcut=True, k=(3, 3), e=0.5):
        """Initializes a standard bottleneck module with optional shortcut connection and configurable parameters."""
        super().__init__()
        c_ = int(c2 * e)  # hidden channels
        self.cv1 = DWSConv(c1, c_, k[0], 1)
        self.cv2 = DWSConv(c_, c2, k[1], 1)
        self.add = shortcut and c1 == c2
...
class C3k2_DWSConv(C2f):
    def __init__(self, c1, c2, n=1, c3k=False, e=0.5, g=1, shortcut=True):
        """Initializes the C3k2 module, a faster CSP Bottleneck with 2 convolutions and optional C3k blocks."""
        super().__init__(c1, c2, n, shortcut, g, e)
        self.m = nn.ModuleList(
            C3k(self.c, self.c, 2, shortcut, g) if c3k else Bottleneck_DWSConv(self.c, self.c, shortcut) for _ in range(n)
        )
